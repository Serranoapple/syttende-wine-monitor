...
PRICE_RE   = re.compile(r'\b(\d{1,2}\.\d{3}|\d{3})(?:,-|\.-)?\b')
    ...
    with pdfplumber.open(BytesIO(pdf_bytes)) as pdf:
        for page in pdf.pages:
            ...
            while i < len(lines):
                ...
                prices = []
                for p in PRICE_RE.findall(context):
                    val = int(p.replace(".", ""))
                    # X.XXX format: altid gyldig pris (1.000-99.999 kr)
                    # XXX format: kun hvis mellem 100-999 kr
                    if "." in p and 1000 <= val <= 99999:
                        prices.append(val)
                    elif "." not in p and 100 <= val <= 999:
                        prices.append(val)
                if not prices:
                    i += 1; continue
