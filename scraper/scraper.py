...
VINTAGE_RE = re.compile(r'\b(19[5-9]\d|20[0-2]\d|NV)\b')
SKIP_RE    = re.compile(
...
DEBUG = os.environ.get("DEBUG_PARSE", "").lower() == "true"

def debug_pdf(pdf_bytes, category):
    """
    Printer de første 3 sider af PDF'en som rå ord med x/y koordinater.
    Kør med DEBUG_PARSE=true for at se hvad pdfplumber faktisk udtrækker.
    """
    print(f"\n{'='*60}")
    print(f"DEBUG: {category}")
    print(f"{'='*60}")
    with pdfplumber.open(BytesIO(pdf_bytes)) as pdf:
        print(f"  Antal sider: {len(pdf.pages)}")
        print(f"  Sidebredde:  {pdf.pages[0].width:.1f} pt")
        print(f"  Sidehøjde:   {pdf.pages[0].height:.1f} pt\n")
        for page_num, page in enumerate(pdf.pages[:3]):
            print(f"  --- Side {page_num+1} ---")
            words = page.extract_words(x_tolerance=3, y_tolerance=3)
            if not words:
                print("  (ingen ord udtrukket -- PDF er sandsynligvis billedbaseret)")
                continue
            # Gruppér på linje
            rows = {}
            for w in words:
            ...
            for y_key in sorted(rows.keys())[:40]:  # Maks 40 linjer per side
                row = sorted(rows[y_key], key=lambda w: w["x0"])
                parts = []
                for w in row:
                    parts.append(f"{w['text']}(x0={w['x0']:.0f},x1={w['x1']:.0f})")
                print(f"  y={y_key:5.0f}  " + "  |  ".join(parts))
            print()

def is_valid_price(token):
    """Returnerer (True, int_value) hvis token er en gyldig pris, ellers (False, None)."""
    m = re.fullmatch(r'(\d{1,2})\.(\d{3})(?:,-|\.-)?' , token)
    if m:
        val = int(m.group(1)) * 1000 + int(m.group(2))
        return (True, val) if 1000 <= val <= 99999 else (False, None)
    m = re.fullmatch(r'(\d{3})(?:,-|\.-)?' , token)
    if m:
        val = int(m.group(1))
        return (True, val) if 100 <= val <= 999 else (False, None)
    return False, None

def is_section_header(words_on_row, page_width):
    """
    En linje er en område-overskrift hvis:
    - Den kun indeholder tekst (ingen priser, ingen årstal)
    - Teksten fylder mindre end 60% af sidebredden (ikke en vin-linje)
    - Ingen ord starter i højre halvdel af siden
    """
    text = " ".join(w["text"] for w in words_on_row)
    if VINTAGE_RE.search(text):
        return False
    prices_found = any(is_valid_price(w["text"])[0] for w in words_on_row)
    if prices_found:
        return False
    max_x = max(w["x1"] for w in words_on_row)
    # Overskrift: tekst slutter inden 55% af sidebredden
    return max_x < page_width * 0.55
...
def parse_pdf(pdf_bytes, category):
    ...
    with pdfplumber.open(BytesIO(pdf_bytes)) as pdf:
        for page in pdf.pages:
            ...
            # Udtræk alle ord med koordinater
            ...
            # Gruppér ord på samme vandrette linje (samme y0, tolerance 3pt)
            rows = {}
            for w in words:
                y_key = round(w["top"] / 3) * 3
                rows.setdefault(y_key, []).append(w)
            ...
            for y_key in sorted(rows.keys()):
                ...
                # ── Område-overskrift? ──────────────────────────────────
                if is_section_header(row_words, page_width):
                    candidate = row_text.strip()
                    # Kun opdater region hvis det ligner et stednavn
                    # (mindst 3 tegn, ingen tal, ikke en kendt skip-linje)
                    if (len(candidate) >= 3
                ...
                # ── Skal linjen have årstal eller NV? ───────────────────
                vintage_match = VINTAGE_RE.search(row_text)
                if not vintage_match:
                    continue   # Ingen årstal/NV → spring over
                vintage = vintage_match.group(1)

                # ── Bottle_price = det HØJESTE x1-koordinat ord der er en gyldig pris ──
                bottle_price = None
                rightmost_x  = -1
                for w in row_words:
                    ok, val = is_valid_price(w["text"])
                    if ok and w["x1"] > rightmost_x:
                        bottle_price = val
                        rightmost_x  = w["x1"]

                if bottle_price is None:
                    continue   # Ingen gyldig pris → spring over

                # ── Glass_price = evt. anden pris til venstre for bottle_price ──
                glass_price = None
                for w in row_words:
                    ok, val = is_valid_price(w["text"])
                    if ok and w["x1"] < rightmost_x:
                ...
                # ── Navn og producent: alle ord til venstre for årstal og priser ──
                vintage_x = next(
                    (w["x0"] for w in row_words if VINTAGE_RE.fullmatch(w["text"])), page_width
                )
                name_words = [
                    w["text"] for w in row_words
                    if w["x1"] <= vintage_x + 5
                    and not is_valid_price(w["text"])[0]
                    and not VINTAGE_RE.fullmatch(w["text"])
                ]
                clean = re.sub(r'\s{2,}', ' ', " ".join(name_words)).strip()
                clean = re.sub(r'[,.\-/|]+$', '', clean).strip()

                if len(clean) < 3:
                    continue

                # Forsøg at splitte producent / vinnavn
